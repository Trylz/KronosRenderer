...
#include "ForwardLighningEffect.h"
#include <dxgi1_4.h>
...
ForwardLighningEffect::ForwardLighningEffect(const SharedDevicePtr& device, const DXGI_SAMPLE_DESC& sampleDesc)
...
ForwardLighningEffect::~ForwardLighningEffect()
{
	for (int i = 0; i < swapChainBufferCount; ++i)
	{
...
void ForwardLighningEffect::initRootSignature()
{
...
void ForwardLighningEffect::initPipelineStateObjects()
{
	// create input layout
...
void ForwardLighningEffect::initStaticConstantBuffers()
{
	HRESULT hr;
	...
	for (int i = 0; i < swapChainBufferCount; ++i)
	{
		...
		memcpy(m_vertexShaderCBGPUAddress[i], &m_vertexShaderCB, sizeof(m_vertexShaderCB));
		...
		ZeroMemory(&m_pixelShaderLightsCB, sizeof(m_pixelShaderLightsCB));

		hr = m_pixelShaderLightsCBUploadHeaps[i]->Map(0, &readRangeGPUOnly, reinterpret_cast<void**>(&m_pixelShaderLightsCBGPUAddress[i]));
		memcpy(m_pixelShaderLightsCBGPUAddress[i], &m_pixelShaderLightsCB, sizeof(PixelShaderLightsCB));
	}
}
...
void ForwardLighningEffect::fromMaterialUploadHeapToDefaulHeap(ID3D12GraphicsCommandList* commandList)
{
	for (int i = 0; i < swapChainBufferCount; ++i)
	{
...
void ForwardLighningEffect::updatePixelShaderLightsCB(ForwardLightningPushArgs& data, int frameIndex)
{
	...
	for (uint32_t i = 0u; i < lightSize; ++i)
	{
		auto& dx12Light = m_pixelShaderLightsCB.lights[i];
		auto lightType = lights[i]->getType();

		dx12Light.type = static_cast<int>(lightType);

		if (lights[i]->getType() == Graphics::Light::LightType::Point)
		{
			auto* omniLight = static_cast<Graphics::Light::OmniLight*>(lights[i].get());

			const auto& pos = omniLight->getPosition();
			dx12Light.position = { pos.x, pos.y, pos.z, 0.0f };
			dx12Light.range = omniLight->getRange();
		}
		else if (lights[i]->getType() == Graphics::Light::LightType::Directionnal)
		{
			auto* directionnalLight = static_cast<Graphics::Light::DirectionnalLight*>(lights[i].get());
			const auto& direction = directionnalLight->getDirection();

			dx12Light.direction = { direction.x, direction.y, direction.z, 0.0f };
		}

		const auto& color = lights[i]->getFinalColor();
...
void ForwardLighningEffect::pushDrawCommands(ForwardLightningPushArgs& data, ID3D12GraphicsCommandList* commandList, int frameIndex)
{
	//Set Pso
