...
CubeMappingEffect::CubeMappingEffect(const SharedDevicePtr& device, const DXGI_SAMPLE_DESC& sampleDesc)
...
void CubeMappingEffect::initRootSignature()
{
...
void CubeMappingEffect::initPipelineStateObjects()
{
...
void CubeMappingEffect::initVertexShaderCB()
{
	HRESULT hr;
...
void CubeMappingEffect::updateVertexShaderCB(CubeMappingPushArgs& data, int frameIndex)
{
	...
	m_vertexShaderCB.cubeMapCenter = { center.x, center.y, center.z};

	memcpy(m_vertexShaderCBGPUAddress[frameIndex], &m_vertexShaderCB, sizeof(VertexShaderCB));
}

void CubeMappingEffect::pushDrawCommands(CubeMappingPushArgs& data, ID3D12GraphicsCommandList* commandList, int frameIndex)
{
